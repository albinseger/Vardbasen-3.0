...
const PersonalTabContent = ({ 
  ...
  console.log("🔹 Personal tab component rendered at", new Date().toLocaleTimeString());
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 p-4 rounded-lg mb-6">
        <p className="text-sm text-blue-700">
          <strong>DEBUG:</strong> Detta är en isolerad komponent för personlig information
        </p>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          onChange={handleChange}
          className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500"
          placeholder="Berätta lite om dig själv, dina intressen och karriärmål..."
        ></textarea>
      </div>
...
export default function StudentRegistration() {
  ...
  const router = {
    push: (url: string) => {
      console.warn("⛔ Router.push intercepted:", url);
      alert(`Navigation attempt intercepted: ${url}`);
      return false;
    },
    back: () => {
      console.warn("⛔ Router.back intercepted");
      return false;
    }
  };
  ...
  useEffect(() => {
    if (activeTab === 'personal') {
      console.log("🔴 Personal tab activated - watching for redirects...");
    }
  }, [activeTab]);
  ...
  const validateEducation = () => {
    ...
    if (!formData.university.trim()) newErrors.university = 'Universitet krävs';
  ...
  const validatePersonal = () => {
    ...
    if (!formData.agreedToTerms) {
      newErrors.agreedToTerms = 'Du måste acceptera villkoren';
    }
  ...
  const switchToTab = (tab: string) => {
    console.log(`🔄 Trying to switch from tab ${activeTab} to ${tab}`);
    
    try {
      if (tab === 'personal') {
        console.log("⚠️ PERSONAL TAB REQUESTED - CAPTURING STACK TRACE");
      ...
      if (activeTab === 'account' && tab !== 'account') {
        ...
        console.log(`Account validation result: ${canProceed ? '✅ Passed' : '❌ Failed'}`);
      } else if (activeTab === 'education' && tab !== 'education' && tab !== 'account') {
        canProceed = validateEducation();
        console.log(`Education validation result: ${canProceed ? '✅ Passed' : '❌ Failed'}`);
      }
      ...
      if (canProceed) {
        ...
        console.log(`❌ Failed to switch to tab ${tab} due to validation errors`);
      }
    } catch (error) {
      console.error("🚨 ERROR IN SWITCH TO TAB:", error);
    }
  };
  ...
  const completeRegistration = async (e: React.FormEvent) => {
    ...
    if (!formData.agreedToTerms) {
      setErrors({
        ...errors,
        agreedToTerms: "Du måste godkänna villkoren för att fortsätta"
      });
  ...
  const manuallyNavigateToProfile = () => {
    console.log("🔵 Manual navigation to profile requested");
  ...
  useEffect(() => {
    ...
    if (personalTabAttempt) {
      console.log("⚠️ Found evidence of previous personal tab attempt at:", personalTabAttempt);
      localStorage.removeItem('personalTabAttempted');
    }
    
    if (personalTabDirectClick) {
      console.log("⚠️ Found evidence of previous direct personal tab click at:", personalTabDirectClick);
      localStorage.removeItem('personalTabDirectClick');
    }
    
    // Global handler to catch form submissions
    const handleFormSubmit = (event: Event) => {
      console.log("⚠️ FORM SUBMISSION DETECTED", event);
      ...
      if (activeTab === 'education') {
        console.warn("🚨 SUSPECT: Form submission from education tab detected");
          ...
          {!isRegistrationComplete ? (
            <>
              <div className="mb-8 text-center">
                <h1 className="text-3xl font-bold text-gray-900">Registrera dig som läkarstudent</h1>
                <p className="text-gray-600 mt-2">
                  Hitta de bästa sommarjobben och gör din profil synlig för arbetsgivare.
                </p>
              </div>
              ...
              <div className="border-b border-gray-200 mb-6">
                <nav className="-mb-px flex space-x-6">
                  ...
                  <button
                    onClick={(e) => {
                      e.preventDefault();
                      console.log("⚠️ Personligt TAB BUTTON CLICKED");
                      try {
                        ...
                        setTimeout(() => {
                          console.log("⏱️ About to switch to personal tab after navigation delay");
              ...
              <div className="bg-white shadow-sm rounded-lg p-6 registration-form-container">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex justify-between pt-4">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          console.log("📋 Nästa: Personligt button clicked");
                          try {
                            ...
                            setTimeout(() => {
                              console.log("⏱️ About to switch to personal tab after delay");
                              switchToTab('personal');
                            }, 100);
                ...
                {activeTab === 'personal' && (
                  <div 
                    ...
                    ref={(el) => {
                      if (el) {
                        console.log("🚧 Personal tab element mounted");
                        
                        // Set timer to detect premature unmounting
                        const mountTime = Date.now();
                        const timeoutId = setTimeout(() => {
                          console.log("✅ Personal tab has been visible for 500ms");
                        }, 500);
                        ...
                        const observer = new MutationObserver((mutations) => {
                          for (const mutation of mutations) {
                            if (mutation.removedNodes.length > 0) {
                              ...
                              console.warn(`⚠️ Personal tab was removed after ${timeVisible}ms`);
                        ...
                        return () => {
                          clearTimeout(timeoutId);
                          observer.disconnect();
                          console.log("🚫 Personal tab element unmounted");
                        };
                      }
                    }}
                  >
            ...
            <div className="bg-white shadow-sm rounded-lg p-6">
              <div className="text-center mb-6">
                ...
                <h2 className="mt-3 text-lg font-medium text-gray-900">Registrering slutförd</h2>
                <p className="mt-2 text-sm text-gray-500">
                  Din profil har skapats framgångsrikt. Klicka på knappen nedan för att gå till din profil.
                </p>
              </div>
